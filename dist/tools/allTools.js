...
import { createIndianInvestmentPlanTool, indianTaxImplicationsTool, portfolioAllocationIndiaTool, riskAssessmentIndianTool, sipGoldPlanningTool, } from "./strategyTools.js";
export const recommendIndianJewellersTool = {
    definition: {
        type: "function",
        function: {
...
async function getIndianJewellers(city) {
    try {
        const allJewellers = [
            {
                name: "Tanishq",
        ...
        if (city) {
            filteredJewellers = allJewellers.filter((j) => j.city.toLowerCase().includes(city.toLowerCase()) ||
                j.city === "Pan India");
        }
        return {
            success: true,
...
export const findIndianGoldSchemesTool = {
    definition: {
        type: "function",
        function: {
            name: "find_indian_gold_schemes",
            description: "Find gold savings schemes, SIPs, and monthly investment plans",
            parameters: {
                type: "object",
                properties: {
                    type: {
...
async function getIndianGoldSchemes(type, city) {
    try {
        const allSchemes = [
            {
                provider: "Tanishq",
        ...
        if (city) {
            filteredSchemes = filteredSchemes.filter((s) => !s.city || s.city.toLowerCase().includes(city.toLowerCase()));
        }
        return {
            success: true,
...
export const locateLocalJewellersTool = {
    definition: {
        type: "function",
        function: {
            name: "locate_local_jewellers",
            description: "Find trusted local jewellers and bullion dealers by city/area",
            parameters: {
                type: "object",
                properties: {
                    city: {
...
async function locateLocalJewellers(city, area) {
    try {
        const localJewellers = {
            delhi: [
                {
        ...
        if (area) {
            filteredJewellers = cityJewellers.filter((j) => j.address.toLowerCase().includes(area.toLowerCase()));
        }
        return {
            success: true,
            data: filteredJewellers,
            message: filteredJewellers.length > 0
...
export const compareGoldLoanTool = {
    definition: {
        type: "function",
        function: {
...
async function compareGoldLoans(loanAmount) {
    try {
        const goldLoanOptions = [
            {
        ...
        if (loanAmount) {
            filteredOptions = goldLoanOptions.filter((option) => loanAmount >= option.minLoanAmount &&
...
export const checkHallmarkTool = {
    definition: {
        type: "function",
        function: {
    ...
    handler: async (params) => {
        return await checkHallmarkCertification(params.certificationNumber, params.jeweller);
    },
};
async function checkHallmarkCertification(certNumber, jeweller) {
    try {
        if (certNumber) {
...
export const getBankGoldOptionsTool = {
    definition: {
        type: "function",
        function: {
...
async function getBankGoldOptions(bank) {
    try {
        const bankGoldOptions = [
            {
        ...
        if (bank) {
            filteredOptions = bankGoldOptions.filter((option) => option.bank.toLowerCase().includes(bank.toLowerCase()));
        }
        return {
            success: true,
            data: filteredOptions,
...
export const calculateIndianFeesTool = {
    definition: {
        type: "function",
        function: {
    ...
    handler: async (params) => {
        return await calculateIndianFees(params.goldPrice, params.quantity, params.makingChargeRate, params.itemType);
    },
};
async function calculateIndianFees(goldPrice, quantity, makingChargeRate, itemType = "jewellery") {
    try {
