...
interface SIPPlan {
	...
	platforms: string[];
	autoInvestmentSchedule: string;
	exitStrategy: string;
}

export const createIndianInvestmentPlanTool: ToolConfig<{
	riskLevel: "low" | "medium" | "high";
	...
	definition: {
		...
		function: {
			...
			description:
				"Generate personalized gold investment strategies for Indian investors",
			parameters: {
				type: "object",
				properties: {
					riskLevel: {
	...
	handler: async (params) => {
		return await createInvestmentPlan(
			params.riskLevel,
...
async function createInvestmentPlan(
	...
	timeHorizon: number,
	monthlyIncome?: number,
): Promise<{ success: boolean; data: InvestmentPlan; message?: string }> {
	try {
		...
		const recommendations = [
			...
			`Diversify across ${Object.keys(allocation).filter((k) => allocation[k as keyof typeof allocation] > 0).length} different gold investment types`,
		...
		return {
			success: false,
			data: {} as InvestmentPlan,
			message: "Failed to create investment plan",
		};
	}
}

export const portfolioAllocationIndiaTool: ToolConfig<{
	riskLevel: "low" | "medium" | "high";
	totalPortfolioValue: number;
	currentGoldHolding?: number;
}> = {
	definition: {
		type: "function",
		function: {
			...
			description:
				"Suggest optimal gold allocation considering Indian investment patterns",
			parameters: {
				type: "object",
				properties: {
					riskLevel: {
	...
	handler: async (params) => {
		return await calculatePortfolioAllocation(
			params.riskLevel,
			params.totalPortfolioValue,
			params.currentGoldHolding,
		);
	},
};

async function calculatePortfolioAllocation(
	riskLevel: "low" | "medium" | "high",
	portfolioValue: number,
	currentGold?: number,
): Promise<{ success: boolean; data: PortfolioAllocation; message?: string }> {
	try {
		let goldPercentage: number;
		let breakdown: PortfolioAllocation["goldAllocation"]["breakdown"];
...
export const riskAssessmentIndianTool: ToolConfig<{
	age: number;
	monthlyIncome: number;
	investmentExperience: "beginner" | "intermediate" | "experienced";
	investmentGoal: "wealth_preservation" | "growth" | "speculation";
	...
	definition: {
		...
		function: {
			...
			description:
				"Assess user's risk tolerance specific to Indian market conditions",
			parameters: {
				type: "object",
				properties: {
					age: {
	...
	handler: async (params) => {
		return await assessRiskProfile(
			params.age,
			params.monthlyIncome,
			params.investmentExperience,
			params.investmentGoal,
			params.liquidityNeeds,
		);
	},
};

async function assessRiskProfile(
	age: number,
	monthlyIncome: number,
	experience: "beginner" | "intermediate" | "experienced",
	goal: "wealth_preservation" | "growth" | "speculation",
	liquidity: "high" | "medium" | "low",
): Promise<{ success: boolean; data: RiskAssessment; message?: string }> {
	try {
		let score = 0;
		...
		if (monthlyIncome >= 200000) score += 3;
		else if (monthlyIncome >= 100000) score += 2;
		else if (monthlyIncome >= 50000) score += 1;
		else score += 0;

		
		if (experience === "experienced") score += 3;
		else if (experience === "intermediate") score += 2;
		else score += 1;

		
		if (goal === "speculation") score += 3;
		else if (goal === "growth") score += 2;
		else score += 1;

		
		if (liquidity === "low") score += 3;
		else if (liquidity === "medium") score += 2;
		else score += 1;

		
		let riskLevel: "low" | "medium" | "high";
		let profile: string;
		let suitableInvestments: string[];
		let warnings: string[];
		let maxGoldAllocation: number;
...
export const indianTaxImplicationsTool: ToolConfig<{
	investmentType?: string;
}> = {
	definition: {
		type: "function",
			...
			parameters: {
				...
				properties: {
					investmentType: {
						...
						description:
							"Specific type of gold investment (optional, shows all if not specified)",
					},
				},
				required: [],
			},
...
async function getIndianTaxImplications(
	investmentType?: string,
): Promise<{ success: boolean; data: TaxImplication[]; message?: string }> {
	try {
		const allTaxInfo: TaxImplication[] = [
			{
				investmentType: "Physical Gold",
				shortTermTax:
					"Added to income, taxed as per income tax slab (30%+ for high earners)",
				...
				gstImplications:
					"3% GST on purchase (input tax credit not available to individuals)",
				importDutyInfo:
					"12.5% basic customs duty + 2.5% agriculture infrastructure cess = 15% total",
			},
			{
			...
			},
			{
				...
				longTermTax:
					"Capital gains completely tax-free if held till maturity (8 years)",
			...
			},
			{
				...
				longTermTax:
					"10% without indexation (after 1 year, if gains > ₹1 lakh)",
			...
			},
			{
				...
				shortTermTax:
					"Added to income, taxed as per slab (treated like physical gold)",
		...
		if (investmentType) {
			...
			const targetType = typeMap[investmentType as keyof typeof typeMap];
			filteredTaxInfo = allTaxInfo.filter(
				(info) => info.investmentType === targetType,
			);
		}
...
export const sipGoldPlanningTool: ToolConfig<{
	riskLevel: "low" | "medium" | "high";
	monthlyAmount: number;
	duration: number; 
	startDate?: string;
...
async function createSIPPlan(
	riskLevel: "low" | "medium" | "high",
	monthlyAmount: number,
	duration: number,
): Promise<{ success: boolean; data: SIPPlan; message?: string }> {
	let investmentMix: SIPPlan["investmentMix"];
	let platforms: string[];
	let autoInvestmentSchedule: string;
